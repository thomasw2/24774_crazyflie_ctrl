...
#include "estimator_kalman_range.h"
#include "system.h"
#include "log.h"
#include "param.h"
...
bool resetEstimationRange = false;
...
void estimatorKalmanRangeTaskInit() {
...
bool estimatorKalmanRangeTaskTest() {
  return isInit;
}
...
static void kalmanTask(void* parameters) {
  ...
  while (true) {
    ...
    if (resetEstimationRange) {
      estimatorKalmanRangeInit();
      paramSetInt(paramGetVarId("kalman", "resetEstimationRange"), 0);
    }
...
void estimatorKalmanRange(state_t *state, const uint32_t tick)
{
...
void estimatorKalmanRangeInit(void)
{
  accAccumulator = (Axis3f){.axis = {0}};
  gyroAccumulator = (Axis3f){.axis = {0}};
...
bool estimatorKalmanRangeTest(void)
{
  return isInit;
}

void estimatorKalmanRangeGetEstimatedPos(point_t* pos) {
  pos->x = coreData.S[KC_STATE_X];
  pos->y = coreData.S[KC_STATE_Y];
  pos->z = coreData.S[KC_STATE_Z];
}

void estimatorKalmanRangeGetEstimatedRot(float * rotationMatrix) {
 ...
 */
  PARAM_ADD_CORE(PARAM_UINT8, resetEstimationRange, &resetEstimationRange)
